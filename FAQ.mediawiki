...
While many of the built in tools now handle periodic boundary conditions cleanly (such as g_dist) with relatively little user interaction, others, such as g_rms, do not. If your simulation analysis protocol requires you to run such a tool, you must correct for the periodic boundary conditions before running it. While there are guidelines available to help you correct for whatever conditions your system may have [http://www.gromacs.org/Documentation/Terminology/Periodic_Boundary_Conditions here], there is an implicit assumption that you have one long running trajectory.
...
<pre>PicklingError: Can't pickle &lt;type 'function'&gt;: attribute lookup __builtin__.function failed</pre>
